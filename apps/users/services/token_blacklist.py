    ...
    def blacklist(access_token_str):
        ...
        try:
            ...
            try:
                ...
                user = None
                if access_token.get(TokenFields.USER_ID.value):
                    try:
                        user = User.objects.get(id=access_token[TokenFields.USER_ID.value])
                    except User.DoesNotExist:
                        user = None
                created_at_aware = datetime.fromtimestamp(access_token[TokenFields.IAT.value], tz=dt_timezone.utc)
                expires_at_aware = datetime.fromtimestamp(access_token[TokenFields.EXP
                                                          .value], tz=dt_timezone.utc)
